...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 27 23:30:03 2024
...
for i in range(0, len(ride_d), batch_size):
    batch = ride_d[i:i + batch_size]
    passenger_data = []
    card_data = []
    for item in batch:
        if len(item['user']) == 18:
            passenger_data.append(tuple(item.get(key, None) for key in key_mapping_passenger.keys()))
        elif len(item['user']) == 9:
            card_data.append(tuple(item.get(key, None) for key in key_mapping_card.keys()))
    cursor.executemany(pr_insert_query, passenger_data)
    cursor.executemany(cr_insert_query, card_data)
